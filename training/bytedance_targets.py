"""MIDI-to-training-target utilities, extracted from ByteDance
piano_transcription (github.com/bytedance/piano_transcription, Apache-2.0).
Contains read_midi() and TargetProcessor unchanged; heavy imports
(h5py/pandas/librosa) trimmed since only mido+numpy are needed here."""
...
def read_midi_any(midi_path):
    """Like read_midi(), but for arbitrary track layouts: merges all tracks in
    playback order (mido handles tempo when iterating a MidiFile), returning
    the same {'midi_event', 'midi_event_time'} format TargetProcessor expects.
    """
    midi_file = MidiFile(midi_path)
    message_list = []
    time_list = []
    ...
    return {
        "midi_event": np.array(message_list),
        "midi_event_time": np.array(time_list),
    }
